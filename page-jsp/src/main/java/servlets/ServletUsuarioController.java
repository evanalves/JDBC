		...
		try {
			...
			if (acao != null && !acao.isEmpty() && acao.equalsIgnoreCase("deletar")) {
				...
				if(modelLogin.getFotouser() != null && !modelLogin.getFotouser().isEmpty()) {
					...
					response.setHeader("Content-Disposition", "attachment;filename=arquivo." + modelLogin.getExtensaofotouser()); /*downloads*/
		...
		try {
			...
			if(ServletFileUpload.isMultipartContent(request)) {
				...
				if(part.getSize() > 0) {
					...
					byte[] foto = IOUtils.toByteArray(part.getInputStream());/*Converte imagem para bytes*/
